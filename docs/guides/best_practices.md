...
**Pythonモジュールを使用する。** ノートブックが長すぎる場合は、複雑なロジックをヘルパーPythonモジュールに分割し、ノートブックにインポートします。Backcastの組み込み[モジュール自動再読み込み](../guides/configuration/runtime_configuration.md#on-module-change)を使用して、モジュールからの変更を自動的にノートブックに取り込みます。
