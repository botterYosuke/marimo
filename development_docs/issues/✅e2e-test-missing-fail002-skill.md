...
**追加日**: 2026-02-21
**追加先**: `frontend/e2e-tests/game/fail.spec.ts`
**テスト名**: 8ケース（初期状態・FAIL_001 unlock/complete・FAIL_002 unlock/complete・ガード2件・全完了）
**テスト結果**: ✅ 8 passed (2.4m)

スキルツリー UI の反応テスト（emitSkillEvent 経由）。FAIL_001/002 の前提条件チェーン、ガード処理、報酬を検証。
NOTE: Python 側の `_check_stop_loss` ロジック（実データ依存）は別途対応が必要。
