...
- [ ] CI への組み込み
- [ ] **UX プレイテストで発見された問題の対応** — 詳細は [ux-playtest-report.md](ux-playtest-report.md)
  - [ ] P1: 再接続時のスキルイベント消失（進捗 0/59 のまま）
  - [ ] P2: `bt.buy()` 出力が `<Order object>` で意味不明
  - [ ] P3: `bt.step()` 出力が `True` のみ
  - [ ] P5: AI Fix バナーがゲームモードで表示される
...
### 37. UX プレイテストで発見されたスキルイベント消失問題（2026-02-20 追加）

**概要**: 初心者視点でのプレイテスト（Playwright MCP でブラウザ操作）により、E2E テストでは検出不可能な UX 問題を 10 件発見。詳細は [ux-playtest-report.md](ux-playtest-report.md) を参照。

**最も深刻な発見 — スキルイベントの消失**:

`bt.chart("7203")` → `bt.buy()` → `bt.step()` を実行後、スキルツリーが **0/59** のまま更新されない。

**発生条件**:
1. marimo サーバー（port 2718）でバックエンドを起動
2. Vite dev server（port 3000）またはブラウザリロードで接続
3. 既存セッションに再接続（Reconnected）
4. セルが自動再実行され `emit_skill()` が発火するが、フロントエンドのスキルリスナーがまだ初期化されていない
5. BroadcastChannel はリアルタイム配信のみで、過去のイベントは再生されない
6. → スキル進捗がフロントエンドに反映されない

**E2E テストとの関係**:
- E2E テスト（案 E）は `window.__testCompleteSkill` テストフックで atom を直接操作するため、この問題を検出しない
- 案 G（`z-python-e2e.spec.ts`）も初回接続から一貫してテストするため再接続シナリオは通らない
- この問題を自動テストするには「再接続後にスキル状態がフロントエンドに正しく同期されるか」を検証する新しいテストスイートが必要

**改善方向**:
1. フロントエンド初期化時に Python カーネルの `get_triggered_skills()` 結果を取得し、スキルツリーと同期
2. `.backcast.progress.json` からの状態復元をフロントエンド初期化時に実行
3. BroadcastChannel リスナー起動後に「過去のスキル状態を再同期するリクエスト」を送信

**その他の主な発見**:
- `bt.buy()` の出力が `<Order object at 0x...>` で初心者に意味不明
