...
class db_manager:
    ...
    def __init__(self):
        ...
        if is_pyodide():
            # WASM環境（Pyodide）の場合
            # marimoのWASMファイルシステム（IndexedDBFS）を使用
            # /marimoディレクトリはIndexedDBFSにマウントされており、
            # このディレクトリ内のファイルは自動的にブラウザストレージ（IndexedDB）に保存される
            self.cache_dir = "/marimo/db"
            # 必要に応じて、メモリ内データベース（:memory:）へのフォールバックも検討
        else:
            # 通常環境の場合（開発・テスト用）
        ...
        # WASM環境ではos.path.exists()が動作しない可能性があるため、
        # エラーハンドリングを追加
        try:
            self.isEnable = True if os.path.exists(self.cache_dir) else False
        except Exception:
            # WASM環境でos.path.exists()が失敗した場合は、ディレクトリ作成を試みる
            try:
