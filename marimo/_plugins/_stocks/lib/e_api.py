...
class e_api:
    ...
    def __init__(self):
        ...
        if is_pyodide():
            # WASM環境の場合
            cache_dir_str = "/marimo/db"
        else:
            # 通常環境の場合
            cache_dir_str = os.path.join(
    ...
    def _set_token(self) -> bool:
        ...
        # 電話
        ## 0120-286-592に電話する
        ## 登録の電話番号から認証電話番号へかけた後、3分以内にログインしてください。
        # return False
...
def _e_normalize_columns(code: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    from marimo._plugins._stocks.lib.stooq import _common_normalize_columns
