...
export const EditGridLayoutRenderer: React.FC<Props> = ({
  layout,
  setLayout,
  cells,
  mode,
  ...
  let grid = (
    ...
    >
      {cells
        ...
        .filter((cell) => !isMarkdownCell(cell.code))
        .map((cell) => {
  ...
  return (
    <div className={cn("relative flex z-10 flex-1 overflow-hidden")}>
      ...
      <div className="flex-none flex flex-col w-[300px] p-2 pb-20 gap-2 overflow-auto bg-(--slate-2) border-t border-x rounded-t shadow-sm transparent-when-disconnected mx-2 mt-4">
        ...
        {notInGrid
          .filter((cell) => {
            // Exclude all markdown cells (they show source + output inline in grid)
...
const GridCell = memo(
  ({
    ...
    if (isOutputEmpty && mode !== "read") {
      return (
        <div ref={containerRef} className={computedClassName}>
          <TinyCode code={code} />
        </div>
      );
    }
