...
export function initialLayoutState(): LayoutState {
  ...
  // 3Dモードでは空のグリッドレイアウトを初期化する。
  // layoutData["grid"] が undefined のままだと getSerializedLayout() が null を返し、
  // バックエンドが layout_file を削除して位置情報が .grid.json に保存されなくなる。
  const layoutData: Partial<Record<LayoutType, LayoutData>> = is3DMode
    ? { grid: GridLayoutPlugin.getInitialLayout([]) }
    : {};
  return {
